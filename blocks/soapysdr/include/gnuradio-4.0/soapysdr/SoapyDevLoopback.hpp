#ifndef GNURADIO_SOAPY_DEV_LOOPBACK_HPP
#define GNURADIO_SOAPY_DEV_LOOPBACK_HPP
 ...
 *  Both RX (SoapyDevSource) and TX (SoapyDevSink) blocks resolve to the same
...
class LoopbackDevice : public SoapySDR::Device {
    ...
    std::atomic<gr::Size_t>                              _simulateOverflowPending{0U}; // queued SOAPY_SDR_OVERFLOW returns from readStream
    std::atomic<gr::Size_t>                              _rxActivateCount{0U};         // observable activate counter for tests
...
public:
    ...
    int activateStream(SoapySDR::Stream* stream, const int /*flags*/ = 0, const long long /*timeNs*/ = 0, const size_t /*numElems*/ = 0) override {
        if (stream == reinterpret_cast<SoapySDR::Stream*>(&_rxStreamSentinel)) {
            ...
            _rxActivateCount.fetch_add(1U, std::memory_order_relaxed);
        } else {
    ...
    int readStream(SoapySDR::Stream* /*stream*/, void* const* buffs, const size_t numElems, int& flags, long long& timeNs, const long timeoutUs = 100000) override {
        ...
        // Test hook: inject SOAPY_SDR_OVERFLOW for the first N reads after
        // simulate_overflow_count is set. Decrements per call.
        if (auto pending = _simulateOverflowPending.load(std::memory_order_relaxed); pending > 0U) {
            _simulateOverflowPending.fetch_sub(1U, std::memory_order_relaxed);
    ...
    void writeSetting(const std::string& key, const std::string& value) override {
        ...
        if (key == "simulate_overflow_count") {
            gr::Size_t n = 0U;
            std::from_chars(value.data(), value.data() + value.size(), n);
            _simulateOverflowPending.store(n, std::memory_order_relaxed);
            return;
        }
        if (key == "device_mode") {
    ...
    std::string readSetting(const std::string& key) const override {
        ...
        if (key == "simulate_overflow_count") {
            return std::to_string(_simulateOverflowPending.load(std::memory_order_relaxed));
        }
        if (key == "rx_activate_count") {
            return std::to_string(_rxActivateCount.load(std::memory_order_relaxed));
        }
        if (key == "device_mode") {
            switch (_deviceMode) {
    ...
    SoapySDR::ArgInfoList getSettingInfo() const override {
        ...
        {
            SoapySDR::ArgInfo info;
            info.key         = "simulate_overflow_count";
            info.value       = "0";
            info.type        = SoapySDR::ArgInfo::INT;
            info.description = "queue N SOAPY_SDR_OVERFLOW returns from readStream (test hook)";
            infos.push_back(info);
        }
        {
            SoapySDR::ArgInfo info;
            info.key         = "rx_activate_count";
            info.value       = "0";
            info.type        = SoapySDR::ArgInfo::INT;
            info.description = "read-only counter of activateStream(rx) calls (test observable)";
            infos.push_back(info);
        }
        {
            SoapySDR::ArgInfo info;
...
#endif // GNURADIO_SOAPY_DEV_LOOPBACK_HPP
