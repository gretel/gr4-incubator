#ifndef GNURADIO_SOAPY_DEV_SINK_HPP
#define GNURADIO_SOAPY_DEV_SINK_HPP
...
#include <gnuradio-4.0/soapysdr/SoapyDevRaiiWrapper.hpp>

namespace gr::incubator::soapysdr {

GR_REGISTER_BLOCK("gr::incubator::soapysdr::SoapyDevSink", gr::incubator::soapysdr::SoapyDevSink, ([T], 1UZ), [ uint8_t, int16_t, std::complex<float> ])
GR_REGISTER_BLOCK("gr::incubator::soapysdr::SoapyDualDevSink", gr::incubator::soapysdr::SoapyDevSink, ([T], 2UZ), [ uint8_t, int16_t, std::complex<float> ])
GR_REGISTER_BLOCK("gr::incubator::soapysdr::SoapyQuadDevSink", gr::incubator::soapysdr::SoapyDevSink, ([T], 4UZ), [ uint8_t, int16_t, std::complex<float> ])

template<typename T, std::size_t nPorts = std::dynamic_extent>
struct SoapyDevSink : Block<SoapyDevSink<T, nPorts>> {
...
the same driver string, enabling full-duplex TX/RX operation.)">;
    ...
    Annotated<gr::Size_t, "channel_offset", Doc<"starting SoapySDR channel index (shifts all per-channel API calls)">>     channel_offset = 0U;
    ...
    Annotated<bool, "timed_tx", Doc<"emit SOAPY_SDR_HAS_TIME on first sample of each burst">>                              timed_tx              = true;
    Annotated<double, "tx_start_delay_s", Unit<"s">, Doc<"delay before first sample for PA settling">>                     tx_start_delay_s      = 0.1;
    ...
    std::atomic<bool>                      _newTxDataReady{false};
    ...
    void start() {
        ...
        _newTxDataReady.store(false, std::memory_order_relaxed);
        _firstWrite = true;
        configureTaper();
        reinitDevice();
        if (!_txStream.get()) {
    ...
    void stop() {
        if (_ioThreadStarted.load(std::memory_order_acquire)) {
            // wake the IO thread if it is currently parked in _newTxDataReady.wait()
            // so it can observe the lifecycle state change and exit its loop.
            _newTxDataReady.store(true, std::memory_order_release);
            _newTxDataReady.notify_one();
            gr::atomic_ref(_ioThreadDone).wait(false);
        }
        if (wait_burst_ack && _txStream.get()) {
            sendEndBurst();
            waitForBurstAck();
        }
        _stagingWriters.clear();
    ...
    work::Result work(std::size_t requestedWork = std::numeric_limits<std::size_t>::max()) noexcept {
        ...
        return Block<SoapyDevSink<T, nPorts>>::work(requestedWork);
    }
    ...
    void ioWriteLoop() {
        ...
        if constexpr (nPorts == 1U) {
            while (lifecycle::isActive(this->state())) {
                ...
                if (avail == 0UZ) {
                    // futex-block instead of busy-yield: avoids burning a core while TX is idle.
                    // processBulk() sets _newTxDataReady after publish() to wake us.
                    _newTxDataReady.wait(false, std::memory_order_acquire);
                    _newTxDataReady.store(false, std::memory_order_relaxed);
                    continue;
                }
                ...
                int       extraFlags = 0;
                long long txTimeNs   = 0LL;
                if (_firstWrite && timed_tx) {
                    extraFlags |= SOAPY_SDR_HAS_TIME;
                    txTimeNs    = static_cast<long long>(_device.getHardwareTime()) + static_cast<long long>(tx_start_delay_s * 1e9);
                    _firstWrite = false;
                }

                auto [written, ok] = taperAndWrite(rSpan.begin(), nActual, txScratch, extraFlags, txTimeNs);
            ...
            while (lifecycle::isActive(this->state())) {
                ...
                auto ret = _txStream.writeStreamFromBufferList(flags, txTimeNs, static_cast<long>(max_time_out_us), std::span<std::span<const T>>(writeSpans));
    ...
    std::pair<std::size_t, bool> taperAndWrite(auto srcIter, std::size_t n, std::vector<T>& scratch, int extraFlags = 0, long long timeNs = 0LL) {
        auto savedPhase   = _taper._phase;
        auto savedRampPos = _taper._rampPosition;
        ...
        auto ret   = _txStream.writeStream(flags, timeNs, static_cast<long>(max_time_out_us), std::span<const T>(scratch.data(), n));
    ...
    void sendEndBurst() {
        // zero-length write with END_BURST flag — many drivers (UHD) require a
        // payload sample to actually flush, so send a single zero sample.
        int                    flags  = SOAPY_SDR_END_BURST;
        long long              timeNs = 0LL;
        const std::array<T, 1> empty{T{0}};
        std::ignore = _txStream.writeStream(flags, timeNs, static_cast<long>(max_time_out_us), std::span<const T>(empty));
    }

    void waitForBurstAck() {
        constexpr std::uint32_t kPollIntervalUs = 10'000U;
        std::uint32_t           elapsed         = 0U;
        while (elapsed < burst_ack_timeout_us) {
            ...
            if (ret == 0 && (flags & SOAPY_SDR_END_BURST)) {
                std::println(stderr, "[SoapySink] BURST_ACK received after {} ms", elapsed / 1000U);
                return;
            }
        ...
        std::println(stderr, "[SoapySink] BURST_ACK timeout after {} us", burst_ack_timeout_us.value);
    }
    ...
    void reinitDevice() {
        ...
        if (nChannelMax < nChannelNeeded + static_cast<std::size_t>(channel_offset.value)) {
            this->emitErrorMessage("reinitDevice()", std::format("TX channel mismatch: need {}+offset {} but device has {}", nChannelNeeded, channel_offset.value, nChannelMax));
            this->requestStop();
            return;
        }

        applyFrontendMapping(); // must precede per-channel settings — defines which channels exist
        applyClockConfig();
        applySampleRate();
        applyAntenna();
        applyFrequency();
    ...
    void applySampleRate() {
        for (gr::Size_t i = 0U; i < num_channels; i++) {
            if (auto r = _device.setSampleRate(SOAPY_SDR_TX, i + channel_offset, static_cast<double>(sample_rate)); !r) {
    ...
    void applyAntenna() {
        ...
        for (gr::Size_t i = 0U; i < num_channels; i++) {
            ...
            if (!ant.empty()) {
                if (auto r = _device.setAntenna(SOAPY_SDR_TX, i + channel_offset, ant); !r) {
    ...
    void applyFrequency() {
        ...
        for (gr::Size_t i = 0U; i < num_channels; i++) {
            ...
            if (auto r = _device.setCenterFrequency(SOAPY_SDR_TX, i + channel_offset, freq); !r) {
    ...
    void applyBandwidth() {
        ...
        for (gr::Size_t i = 0U; i < num_channels; i++) {
            ...
            if (auto r = _device.setBandwidth(SOAPY_SDR_TX, i + channel_offset, bw); !r) {
    ...
    void applyGain() {
        ...
        for (gr::Size_t i = 0U; i < num_channels; i++) {
            ...
            if (auto r = _device.setGain(SOAPY_SDR_TX, i + channel_offset, g); !r) {
    ...
    void applyGainMode() {
        ...
        for (gr::Size_t i = 0U; i < num_channels; i++) {
            if (!_device.hasAutomaticGainControl(SOAPY_SDR_TX, i + channel_offset)) {
                continue;
            }
            if (auto r = _device.setAutomaticGainControl(SOAPY_SDR_TX, i + channel_offset, gain_mode); !r) {
    ...
    void applyFrequencyCorrection() {
        ...
        for (gr::Size_t i = 0U; i < num_channels; i++) {
            if (!_device.hasFrequencyCorrection(SOAPY_SDR_TX, i + channel_offset)) {
                continue;
            }
            if (auto r = _device.setFrequencyCorrection(SOAPY_SDR_TX, i + channel_offset, frequency_correction); !r) {
    ...
    void applyDcOffsetMode() {
        ...
        for (gr::Size_t i = 0U; i < num_channels; i++) {
            if (!_device.hasDCOffsetMode(SOAPY_SDR_TX, i + channel_offset)) {
                continue;
            }
            if (auto r = _device.setDCOffsetMode(SOAPY_SDR_TX, i + channel_offset, dc_offset_mode); !r) {
    ...
    void applyDcOffset() {
        ...
        for (gr::Size_t i = 0U; i < num_channels; i++) {
            if (!_device.hasDCOffset(SOAPY_SDR_TX, i + channel_offset)) {
                continue;
            }
            ...
            if (auto r = _device.setDCOffset(SOAPY_SDR_TX, i + channel_offset, dc_offset->at(idx), dc_offset->at(idx + 1UZ)); !r) {
    ...
    void applyIqBalance() {
        ...
        for (gr::Size_t i = 0U; i < num_channels; i++) {
            if (!_device.hasIQBalance(SOAPY_SDR_TX, i + channel_offset)) {
                continue;
            }
            auto idx = static_cast<std::size_t>(i) * 2UZ;
            ...
            if (auto r = _device.setIQBalance(SOAPY_SDR_TX, i + channel_offset, iq_balance->at(idx), iq_balance->at(idx + 1UZ)); !r) {
...
template<typename T>
using SoapySimpleSink = SoapyDevSink<T, 1UZ>;
template<typename T>
using SoapyDualSink = SoapyDevSink<T, 2UZ>;
template<typename T>
using SoapyQuadSink = SoapyDevSink<T, 4UZ>;

} // namespace gr::incubator::soapysdr

#endif // GNURADIO_SOAPY_DEV_SINK_HPP
