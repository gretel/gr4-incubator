#ifndef SOAPY_DEV_RAII_WRAPPER_HPP
#define SOAPY_DEV_RAII_WRAPPER_HPP
...
public:
    ...
    public:
        ...
        [[maybe_unused]] int readStreamStatus(std::size_t& chanMask, int& flags, long long& timeNs, long timeOutUs)
        requires(Direction == SOAPY_SDR_TX)
        {
            return SoapySDRDevice_readStreamStatus(_device.get(), _stream.get(), &chanMask, &flags, &timeNs, timeOutUs);
        }
...
struct std::formatter<SoapySDRRange> {
    ...
    auto format(const gr::incubator::soapysdr::Range& range, FormatContext& ctx) const noexcept {
        return std::format_to(ctx.out(), "Range{{min: {}, max: {}, step: {}}}", range.minimum, range.maximum, range.step);
    }
};

#endif // SOAPY_DEV_RAII_WRAPPER_HPP
