#ifndef GNURADIO_SOAPY_DEV_SOURCE_HPP
#define GNURADIO_SOAPY_DEV_SOURCE_HPP
...
GR_REGISTER_BLOCK("gr::incubator::soapysdr::SoapyDevSource", gr::incubator::soapysdr::SoapyDevSource, ([T], 1UZ), [ uint8_t, int16_t, std::complex<float> ])
GR_REGISTER_BLOCK("gr::incubator::soapysdr::SoapyDualDevSource", gr::incubator::soapysdr::SoapyDevSource, ([T], 2UZ), [ uint8_t, int16_t, std::complex<float> ])

template<typename T, std::size_t nPorts = std::dynamic_extent>
struct SoapyDevSource : Block<SoapyDevSource<T, nPorts>> {
...
Tested with RTL-SDR and LimeSDR drivers.)">;
    ...
    Annotated<double, "lo_offset", Unit<"Hz">, Doc<"LO offset for DC spur avoidance (passed as OFFSET tune arg)">>               lo_offset            = 0.0;
    ...
    Annotated<std::string, "overflow_recovery", Doc<"overflow action: 'reactivate' (deactivate+activate) or 'log_only' (tag+continue)">>                                          overflow_recovery             = std::string("reactivate");
    Annotated<gr::Size_t, "overflow_reactivate_threshold", Doc<"consecutive overflows before reactivation (0 or 1 = every overflow; ignored when overflow_recovery='log_only')">> overflow_reactivate_threshold = 1U;
    ...
    gr::Size_t                             _consecutiveOverflows{0U}; // IO-thread only (handleStreamError + ioReadLoop reset)
    ...
    void settingsChanged(const property_map& /*oldSettings*/, property_map& newSettings, property_map& forwardSettings) {
        if (newSettings.contains("overflow_recovery")) {
            const auto& mode = overflow_recovery.value;
            if (mode != "reactivate" && mode != "log_only") {
                this->emitErrorMessage("settingsChanged()", std::format("invalid overflow_recovery '{}': expected 'reactivate' or 'log_only'", mode));
            }
        }
        if (!_device.get()) {
            return;
        }
        if (newSettings.contains("frequency") || newSettings.contains("lo_offset")) {
            applyFrequency();
            if (!frequency->empty()) {
    ...
    void start() {
        ...
        _newDataReady.store(false, std::memory_order_relaxed);
        ...
        detail::DeviceRegistry::registerActivation(_devKwargs, [this] {
            ...
            // in a single streamer will fail to time align."
            std::size_t                    nCh = (nPorts != std::dynamic_extent) ? nPorts : static_cast<std::size_t>(num_channels.value);
            std::expected<void, gr::Error> r;
            if (nCh > 1UZ) {
                auto                now           = static_cast<long long>(_device.getHardwareTime());
                constexpr long long kStartDelayNs = 100'000'000LL; // 100 ms
                auto                startTime     = now + kStartDelayNs;
                std::println(stderr, "[SoapySource] timed RX activate: now={} start={} (+{} ms)", now, startTime, kStartDelayNs / 1'000'000LL);
                r = _rxStream.activate(SOAPY_SDR_HAS_TIME, startTime);
            } else {
                r = _rxStream.activate();
            }
            if (!r) {
    ...
    work::Result work(std::size_t requestedWork = std::numeric_limits<std::size_t>::max()) noexcept {
        ...
        // Block until the IO thread has published new data, avoiding scheduler
        // busy-spin.  Returning {0, OK} from work() lets the scheduler treat the
        // call as "no progress", which combined with progress->incrementAndGet()
        // from the IO thread reset the inactivity counter on every chunk and
        // prevented the worker from ever sleeping — costing ~110% CPU on Apple
        // Silicon.  Mirrors the natural throttle of the old synchronous
        // processBulk model where readStream() blocking was the limiter.
        if (_newDataReady.exchange(false, std::memory_order_acquire)) {
            return {requestedWork, 1UZ, work::Status::OK};
        }
        _newDataReady.wait(false, std::memory_order_acquire);
        _newDataReady.store(false, std::memory_order_relaxed);
        return {requestedWork, 1UZ, work::Status::OK};
    }
    ...
    void ioReadLoop() {
        ...
        if constexpr (nPorts == 1U) {
            ...
            while (lifecycle::isActive(this->state())) {
                ...
                span.publish(nCopy);
                _newDataReady.store(true, std::memory_order_release);
                _newDataReady.notify_one();
        ...
        // unblock work() if it is currently waiting on _newDataReady
        _newDataReady.store(true, std::memory_order_release);
        _newDataReady.notify_one();
    }
    ...
    void applyFrequency() {
        ...
        for (gr::Size_t i = 0U; i < num_channels; i++) {
            ...
            if (auto r = _device.setCenterFrequency(SOAPY_SDR_RX, i, freq, tuneArgs); !r) {
    ...
    bool handleStreamError(int ret) {
        ...
        case SOAPY_SDR_OVERFLOW: {
            ...
            // Log every overflow when verbose_overflow=true; otherwise log only the
            // first overflow of a burst (consecutive==1) so prod logs get one
            // breadcrumb per burst without per-sample spam.
            if (verbose_overflow || _consecutiveOverflows == 1U) {
                std::println(stderr, "[SoapySource] OVERFLOW #{} (consecutive: {})", count, _consecutiveOverflows);
            }
            emitOverflowTag();
            ...
            // log_only mode: emit tag but skip expensive deactivate/reactivate cycle
            if (overflow_recovery != "reactivate") {
                return true;
            }
            // threshold gating: only reactivate after N consecutive overflows
            if (overflow_reactivate_threshold > 1U && _consecutiveOverflows < overflow_reactivate_threshold) {
                return true;
            }
            _consecutiveOverflows = 0U;
            if (auto r = _rxStream.deactivate(); !r) {
            ...
            // Multi-channel RX requires timed reactivation for channel
            // alignment — same logic as the initial activation in start().
            // Without SOAPY_SDR_HAS_TIME, UHD rejects stream_now on
            // multi-channel streamers ("will fail to time align").
            std::size_t                    nCh = (nPorts != std::dynamic_extent) ? nPorts : static_cast<std::size_t>(num_channels.value);
            std::expected<void, gr::Error> activateResult;
            if (nCh > 1UZ && _device.get()) {
                auto                now           = static_cast<long long>(_device.getHardwareTime());
                constexpr long long kStartDelayNs = 100'000'000LL; // 100 ms
                auto                startTime     = now + kStartDelayNs;
                if (verbose_overflow) {
                    std::println(stderr, "[SoapySource] OVERFLOW #{}: timed reactivate", count);
                }
                activateResult = _rxStream.activate(SOAPY_SDR_HAS_TIME, startTime);
            } else {
                if (verbose_overflow) {
                    std::println(stderr, "[SoapySource] OVERFLOW #{}: reactivate", count);
                }
                activateResult = _rxStream.activate();
            }
            if (!activateResult) {
                this->emitErrorMessage("ioReadLoop()", activateResult.error());
                this->requestStop();
                return false;
            }
            return true;
...
template<typename T>
using SoapySimpleSource = SoapyDevSource<T, 1UZ>;
template<typename T>
using SoapyDualSource = SoapyDevSource<T, 2UZ>;

} // namespace gr::incubator::soapysdr

#endif // GNURADIO_SOAPY_DEV_SOURCE_HPP
