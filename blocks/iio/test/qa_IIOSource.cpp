...
const suite IIOSourceMimoTests = [] {
    "nPorts=2 default property values"_test = [] {
    ...
    "nPorts=2 int16 specialisation default constructible"_test = [] {
        IIOSource<std::complex<std::int16_t>, 2> blk;
        expect(blk.uri == "local:");
        expect(blk.channels.size() == 4_u);
    };

    "nPorts=2 settingsChanged is no-op pre-start"_test = [] {
        IIOSource<std::complex<float>, 2> blk;
        gr::property_map                   empty_old;
        gr::property_map                   new_settings{
            {"center_frequency", 900'000'000.0},
    ...
    "nPorts=2 wrong channels size throws on start"_test = [] {
        IIOSource<std::complex<float>, 2> blk;
    ...
    "nPorts=2 wrong rx_chains size throws on start"_test = [] {
        IIOSource<std::complex<float>, 2> blk;
        blk.uri        = "ip:127.0.0.1:1";
    ...
    "nPorts=2 bad uri throws on start"_test = [] {
        IIOSource<std::complex<float>, 2> blk;
        blk.uri        = "ip:127.0.0.1:1";
        blk.timeout_ms = 100U;
        expect(throws([&] { blk.start(); }));
    ...
    "nPorts=2 stop on never-started block is safe"_test = [] {
        IIOSource<std::complex<float>, 2> blk;
        expect(nothrow([&] { blk.stop(); }));
    };

    // Live-device smoke: opt-in via FISH_BALL_URI=ip:<host>.
    "FISH_BALL_URI nPorts=2 live reinit"_test = [] {
